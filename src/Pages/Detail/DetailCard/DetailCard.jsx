...
import useDetailCardModel from "../../../Models/DetailModel/DetailCardModel/DetailCardModel";

const DetailCard = () => {
  const { data, loading, sortOption, setSortOption } = useDetailCardModel();
  const [selectedFilters, setSelectedFilters] = useState({ division: [], district: [] });
  ...
  const filteredData = () => {
    return data.filter(item => {
      const divisionMatch = selectedFilters.division.length === 0 || selectedFilters.division.includes(item.division);
      const districtMatch = selectedFilters.district.length === 0 || selectedFilters.district.includes(item.district);
        ...
        <div className="flex flex-col w-full">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pl-4">
            {filteredData().map((item) => (
              <div key={item.id} className="border rounded-lg shadow-md p-4 bg-white transition-transform duration-300 transform">
                <div className="flex flex-col">
                  ...
                  <p className="hidden md:block text-gray-600 text-base my-2 line-clamp-5 overflow-hidden">
                    {item.description?.length > 135 ? `${item.description.substring(0, 135).trim().replace(/[,;:.!?]$/, "")}...` : item.description || ""}
                  </p>
